...
from comments.models import Comment_Community
...
def index(request, tag=None):
    ...
    # 커뮤니티
    communitys_obj = Community.objects.order_by('attribute', '-upload_date').filter(is_published=True)
    board_kind = 'all'
    # 상단태그맵핑 클릭시
    if tag == 'free':
        communitys_obj = communitys_obj.filter(board_kind='자유게시판')
        board_kind = 'free'
    elif tag == 'visit':
        communitys_obj = communitys_obj.filter(board_kind='방문인사')
        board_kind = 'visit'
    elif tag == 'withme':
        communitys_obj = communitys_obj.filter(board_kind='동행')
        board_kind = 'withme'
    elif tag == 'buysell':
        communitys_obj = communitys_obj.filter(board_kind='사고팔기')
        board_kind = 'buysell'
    ...
    if s_select != None:
        if s_select == '제목':
            communitys_obj = communitys_obj.filter(title__contains=s_text)
        else :
            communitys_obj = communitys_obj.filter(user__username__contains=s_text)

    # 페이지네이션
    paginator = Paginator(communitys_obj, 20)
    page = request.GET.get('page')
    communitys = paginator.get_page(page)
    # 페이지네이션 나타낼 번호 제어
    page_range = pagination_range(communitys, paginator)
    # 우측레이아웃데이터
    rights = right_index()
    ...
    return render(request, 'communitys/index.html', context)

def show(request, community_id, comment_id=None):
    # 배너 데이터
    banners = community_banners()
    #급상승키워드
    upkeyword = uprising_keyword()
    ...
    # 게시글의 댓글 데이터
    comments = community.comment_community_set.all().order_by('upload_date').filter(is_published=True)
    # 게시글의 태그 데이터
    board_kind = community.board_kind
    # 댓글쓸시 포인트
    point = Point.objects.filter(title='커뮤니티댓글').values('point').first()

    # 게시글에 대한 회원의 좋아요 여부
    if request.user.id != None:
        addlike = AddLike.objects.filter(user=request.user).filter(content_name='커뮤니티').filter(content_id=community_id).filter(is_active=True)
        if addlike.exists():
            addlike = addlike[0]
    else :
        addlike = AddLike.objects.filter(user=None).filter(content_name='커뮤니티').filter(content_id=community_id).filter(is_active=True)

    # 해당 게시글의 댓글들에 대한 회원의 좋아요 여부
    if request.user.id != None:
        c_addlikes = AddLike.objects.filter(user=request.user).filter(content_name='커뮤니티댓글').filter(content_id=community_id).filter(is_active=True).values_list('comment_id',flat=True)
    ...
    return render(request, 'communitys/show.html', context)
...
def create(request, tag=None):
    ...
    rights = right_index()
    # 글쓸시 포인트, 경험치
    point = Point.objects.filter(title='커뮤니티작성').values('point').first()
    ex = Exprience.objects.filter(title='커뮤니티작성').values('exprience').first()
    ...
    if request.method == 'POST':
        ...
        show_ad = Banner.objects.get(title='커뮤니티상세보기배너_1')
        ...
        if val_message != '유효성검사성공':
            ...
            return render(request, 'communitys/create.html', context)

        # 임시이미지 실제이미지로
        pattern = re.compile ('<img [^>]*src="([^"]+)')
        old_img = pattern.findall(content)
        # Content에 실제 이미지파일로 맵핑 바꾸기
        content = content.replace('/media/communitys/temp/', '/media/communitys/communitys/')
        ...
        community.save()
        return redirect('communitys:index')
...
def edit(request, community_id):
    ...
    if request.method == 'POST':
        ...
        # 유효성 검사 실패시 기존데이터 저장
        old_data = {'id':community.id,'tag':tag, 'title':title, 'content':content, 'g_map1':g_map1, 'g_map2':g_map2, 'g_map3':g_map3,
        ...
        if val_message != '유효성검사성공':
            messages.error(request, val_message)
            return render(request, 'communitys/edit.html', context)
        ...
        community.user, community.board_kind = user, board_kind
        community.tag, community.title, community.content = tag, title, content
        community.g_map1, community.g_map2, community.g_map3, community.g_map4, community.g_map5 = g_map1, g_map2, g_map3, g_map4, g_map5
        community.url_link, community.is_imaged = url_link, is_imaged
        community.save()
        return redirect('communitys:index')
    else:
        return render(request, 'communitys/edit.html', context)
...
def right_index():
    ...
    right_bests = Community.objects.filter(upload_date__gte=last_month).order_by('-views').filter(is_published=True)
    right_news = Community.objects.order_by('-upload_date').filter(is_published=True)
    right_comments = Comment_Community.objects.order_by('-upload_date').filter(is_published=True)
    rights = {
...
# 페이지네이션
def pagination_range(communitys, paginator):
    index = communitys.number
...
def summernote_tmp(request):
    if request.method == 'POST' and request.FILES['file']:
        ...
        destination = settings.MEDIA_ROOT+'/communitys/temp/'
        ...
        if ext == '.png' or ext == '.jpg' or ext == '.PNG' or ext == '.JPG':
            ...
            file_url = '/media/communitys/temp/'+filename
...
def add_like(request):
    ...
    c_alarm = Alarm_Addcommentlike.objects.create(
        act_type = 'like', likefrom=request.user, comment_user=community.user, comment_table='커뮤니티',
        board_name='커뮤니티',board_url='communitys/show/'+str(content_id), content_id = content_id
    )
    c_alarm.save()
...
def c_add_like(request):
    ...
    # 댓글-좋아요 알람생성
    c_alarm = Alarm_Addcommentlike.objects.create(
        act_type = 'c_like', likefrom=request.user, comment_user=comment.user, comment_table='커뮤니티댓글',
        board_name='커뮤니티',  board_url='communitys/show/'+str(content_id)+'/'+str(comment_id), content_id = content_id, comment_id = comment_id
    )
    c_alarm.save()
...
def c_add_unlike(request):
    ...
    comment = get_object_or_404(Comment_Community, pk=comment_id)
    addLike = AddLike.objects.filter(user=request.user, content_name="커뮤니티댓글", content_id=content_id, comment_id=comment_id).filter(is_active=True).get()
...
def community_banners():
    ...
    banners_review_left = Banner.objects.order_by('position_1').filter(title__contains='커뮤니티좌측').filter(is_published=True)
    banners_review_right = Banner.objects.order_by('position_1').filter(title__contains='커뮤니티우측').filter(is_published=True)
    # 쇼핑배너
