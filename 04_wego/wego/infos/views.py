...
from comments.models import Comment_Info
...
def index(request, tag=None):
    ...
    # 이용후기
    infos_obj = Info.objects.order_by('attribute', '-upload_date').filter(is_published=True)
    board_kind = 'all'
    # 상단태그맵핑 클릭시
    if tag == 'best':
        infos_obj = infos_obj.filter(best_published=True)
        board_kind = 'best'
    elif tag == 'prepare':
        infos_obj = infos_obj.filter(board_kind='여행준비')
        board_kind = 'prepare'
    elif tag == 'cebutip':
        infos_obj = infos_obj.filter(board_kind='세부팁')
        board_kind = 'cebutip'
    ...
    if s_select != None:
        if s_select == '제목':
            infos_obj = infos_obj.filter(title__contains=s_text)
        else :
            infos_obj = infos_obj.filter(user__username__contains=s_text)

    # 페이지네이션
    paginator = Paginator(infos_obj, 10)
    page = request.GET.get('page')
    infos = paginator.get_page(page)
    # 페이지네이션 나타낼 번호 제어
    page_range = pagination_range(infos, paginator)
    # 우측레이아웃데이터
    rights = right_index()
...
def show(request, info_id, comment_id=None):
    # 배너 데이터
    banners = info_banners()
    #급상승키워드
    upkeyword = uprising_keyword()
    ...
    # 게시글의 댓글 데이터
    comments = info.comment_info_set.all().order_by('upload_date').filter(is_published=True)
    # 게시글의 태그 데이터
    board_kind = info.board_kind
    # 댓글쓸시 포인트
    point = Point.objects.filter(title='정보공유댓글').values('point').first()

    # 게시글에 대한 회원의 좋아요 여부
    if request.user.id != None:
        addlike = AddLike.objects.filter(user=request.user).filter(content_name='정보공유').filter(content_id=info_id).filter(is_active=True)
        if addlike.exists():
            addlike = addlike[0]
    else :
        addlike = AddLike.objects.filter(user=None).filter(content_name='정보공유').filter(content_id=info_id).filter(is_active=True)

    # 해당 게시글의 댓글들에 대한 회원의 좋아요 여부
    if request.user.id != None:
        c_addlikes = AddLike.objects.filter(user=request.user).filter(content_name='정보공유댓글').filter(content_id=info_id).filter(is_active=True).values_list('comment_id',flat=True)
    ...
    return render(request, 'infos/show.html', context)
...
def create(request, tag=None):
    ...
    rights = right_index()
    # 글쓸시 포인트, 경험치
    point = Point.objects.filter(title='정보공유작성').values('point').first()
    ex = Exprience.objects.filter(title='정보공유작성').values('exprience').first()
    ...
    if request.method == 'POST':
        ...
        show_ad = Banner.objects.get(title='정보공유상세보기배너_1')
        ...
        content = content.replace('/media/infos/temp/', '/media/infos/infos/')
...
def edit(request, info_id):
    ...
    if request.method == 'POST':
        ...
        # 유효성 검사 실패시 기존데이터 저장
        old_data = {'id':info.id,'tag':tag, 'title':title, 'content':content, 'g_map1':g_map1, 'g_map2':g_map2, 'g_map3':g_map3,
            'g_map4':g_map4, 'g_map5':g_map5, 'url_link':url_link, 'board_kind':board_kind}
        context['old_data'] = old_data
        ...
        info.user, info.board_kind = user, board_kind
        info.index_image1, info.index_image2, info.index_image3, info.index_image4, info.index_image5 = index_image[0], index_image[1], index_image[2], index_image[3], index_image[4]
        info.index_content, info.tag, info.title, info.content = index_content, tag, title, content
        info.g_map1, info.g_map2, info.g_map3, info.g_map4, info.g_map5 = g_map1, g_map2, g_map3, g_map4, g_map5
        info.url_link = url_link
        info.save()
        return redirect('infos:index')
    else:
        return render(request, 'infos/edit.html', context)
...
def chatting(request):
    ...
    return render(request, 'infos/chatting.html', context)

# 우측 레이아웃관련 데이터
def right_index():
    right_bests = Info.objects.filter(best_published=True).order_by('-upload_date').filter(is_published=True)
    right_news = Info.objects.order_by('-upload_date').filter(is_published=True)
    right_comments = Comment_Info.objects.order_by('-upload_date').filter(is_published=True)
    rights = {
...
# 페이지네이션
def pagination_range(infos, paginator):
    index = infos.number
...
def add_like(request):
    ...
    c_alarm = Alarm_Addcommentlike.objects.create(
        act_type = 'like', likefrom=request.user, comment_user=info.user, comment_table='정보공유',
        board_name='정보공유',board_url='infos/show/'+str(content_id), content_id = content_id
    )
    c_alarm.save()
...
def c_add_like(request):
    ...
    # 댓글-좋아요 알람생성
    c_alarm = Alarm_Addcommentlike.objects.create(
        act_type = 'c_like', likefrom=request.user, comment_user=comment.user, comment_table='정보공유댓글',
        board_name='정보공유',  board_url='infos/show/'+str(content_id)+'/'+str(comment_id), content_id = content_id, comment_id = comment_id
    )
    c_alarm.save()
...
def c_add_unlike(request):
    ...
    comment = get_object_or_404(Comment_Info, pk=comment_id)
    addLike = AddLike.objects.filter(user=request.user, content_name="정보공유댓글", content_id=content_id, comment_id=comment_id).filter(is_active=True).get()
...
def summernote_tmp(request):
    if request.method == 'POST' and request.FILES['file']:
        ...
        destination = settings.MEDIA_ROOT+'/infos/temp/'
        ...
        if ext == '.png' or ext == '.jpg' or ext == '.PNG' or ext == '.JPG':
            ...
            file_url = '/media/infos/temp/'+filename
...
def info_banners():
    ...
    banners_review_left = Banner.objects.order_by('position_1').filter(title__contains='정보공유좌측').filter(is_published=True)
    banners_review_right = Banner.objects.order_by('position_1').filter(title__contains='정보공유우측').filter(is_published=True)
